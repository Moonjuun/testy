...
export function TestHistoryPreview({ test, onBack }: TestHistoryPreviewProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 shadow space-y-4">
        {/* 카드 내부 상단에 백 버튼 배치 (여백 최소화) */}
        <div className="mb-1">
          <Button
            onClick={onBack}
            variant="ghost"
            size="sm"
            className="flex items-center gap-1 px-0 text-sm text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white"
          >
        ...
        <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
          ...
          {test.result.keywords?.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {test.result.keywords.map((keyword: string, index: number) => (
                <span
                  key={index}
                  className="bg-purple-200 text-purple-800 dark:bg-purple-800 dark:text-purple-200 px-3 py-1 rounded-full text-xs font-semibold shadow-sm"
                >
                  #{keyword}
                </span>
              ))}
