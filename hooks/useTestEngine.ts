...
export function useTestEngine(testData: TestData) {
  ...
  const handleNext = useCallback(() => {
    if (selectedOption !== null) {
      ...
      newAnswers[currentQuestion] = testData.questions[currentQuestion].options[selectedOption].score
  ...
  const calculateResult = useCallback((): TestResult => {
    ...
    const result = testData.results.find(
      (result) => totalScore >= result.score_range[0] && totalScore <= result.score_range[1],
    )
