...
export async function generateImageWithGemini(
  ...
  if (!projectId) {
    ...
    console.error("💡 Imagen 4.0은 Vertex AI API를 사용하므로 프로젝트 ID가 필요합니다.");
    return null;
  }
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (response.status === 401) {
        ...
        console.error("💡 GOOGLE_ACCESS_TOKEN 환경 변수를 설정하거나 서비스 계정 키를 설정하세요.");
        return null;
      }
      ...
      if (error.message?.includes("404") || error.message?.includes("not found")) {
        ...
        console.error("💡 Vertex AI에서 Imagen API를 활성화하거나, 모델 이름을 확인하세요.");
        return null;
      }
